...
{
    /**触摸层*/
    private touchLayer: egret.TextField;//这个用来响应特定区域的触摸事件
    /**地图*/
    private ditu: CreateMap = new CreateMap();
    /**加载*/
    ...
    /**武器池*/
    private WeaponPool_0_1: WeaponPool = new WeaponPool();
    /**武器*/
    private Weapon_0_1: Weapon = new Weapon();
    /**角色属性*/
    private roleClass: RoleClass = ConstantA.roleClass;
    /**主角标题*/
    private roleTitle: RoleTitle = new RoleTitle();
    /**主角初始的 XY*/
    private startPoint: Point = new Point();
    /**主角的 XY*/
    private downPoint: Point = new Point();
    /**主角的 角速度*/
    ...
    /**地图X坐标偏移*/
    private offset: number = ConstantA.stageW / 2;
    /**攻击事件定时器*/
    private GongJiFrame: egret.Timer = new egret.Timer( 1000 / 2 );
    /**首次加载角色是否已经创建完毕*/
    private isCreateRo: boolean = false;
    /**升级经验条*/
    private eUI: expUI = new expUI();
    ...
    /**响应 结束游戏按钮1 的事件*/
    private onButtonA_Click( event: egret.TouchEvent ): void
    {
        //删除所有侦听器（会不会是多此一举呢？）
    ...
    /**响应 复活按钮2 的事件*/
    private onButtonB_Click( event: egret.TouchEvent ): void
    {
    ...
    /**响应 商城按钮3 的事件*/
    private onButtonC_Click( event: egret.TouchEvent ): void
    {
        //获取音乐文件
    ...
    /**响应 切换地图 按钮4 的事件*/
    private onButtonD_Click( event: egret.TouchEvent ): void
    {
        egret.gui.Alert.show( "暂未开发\n敬请期待！" );
    }
    ...
    private onRes( e: egret.Event )
    {
        ...
        {
            var A: number = this.roleClass.map;
            if ( this.ditu.parent )
            {
                //this.ditu.removeChildren();
            }
            else
            {
        ...
        if ( e.type == "stopRes" )
        {
            ...
            if ( !this.eUI.parent )
            {
                ...
                var lvExp: number = parseInt( ConstantA.levelExp["" + ( this.roleClass.level + 1 )] );//升级所需经验
    ...
    */
    ...
    {
        //计算出 x 、 y速度
        var vx: number = -Math.round( Math.cos( this.angleSpeed ) * this.speed );
        //嗷嗷嗷....这里个VX 正负怎么会反过来的呢？？暂时不理了
        this.ditu.setMapX( vx );
        this.offset -= vx;
    }
    /**响应 触摸放开 事件*/
    private onStop( e: egret.TouchEvent )
    {
