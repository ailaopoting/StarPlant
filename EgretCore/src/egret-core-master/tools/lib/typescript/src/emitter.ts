...
namespace ts {
    ...
    export function emitFiles(resolver: EmitResolver, host: EmitHost, targetSourceFile: SourceFile, program:Program): EmitResult {
    ...
    return new Promise(function (resolve, reject) {
        ...
        function emitJavaScript(jsFilePath: string, root?: SourceFile) {
            ...
            function emitFunctionDeclaration(node: FunctionLikeDeclaration) {
                var name:any;
                if (global["ignoreDollar"] && node.name && (name = (<any>node.name)['text']) && name.indexOf('$')==0)
                    return;
            ...
            function emitClassLikeDeclarationBelowES6(node: ClassLikeDeclaration) {
                ...
                function getImplementedInterfaces(node: Node, names: any, isClass = true) {
                    ...
                    if (isClass)
                        superInterfaces = ts.getClassImplementsHeritageClauseElements(<ClassLikeDeclaration>node);
                    else
